...
long int cal(int i, int j, int n, int b, vector<vector<long int>>& dp) {
    if(i >= n || j >= n)
        return 0;
    int m = n / 2;
    if(i == m && j == m)
    ...
    if(dp[i][j] != -1)
        return dp[i][j];
    long int down = cal(i + 1, j, n, b, dp);
    long int right = cal(i, j + 1, n, b, dp);
    return dp[i][j] = down + right;
}
int solve(int A, int B) {
    vector<vector<long int>> dp(A, vector<long int> (A, -1));
    long int ans = cal(0, 0, A, B, dp);
    int mod = 1e9 + 7;
    return ans % mod;
}
