...
// if any bit position we have 1 or more one(no matter even times or odd using the nums[i] AND (nums[i] XOR x) 
// operation we can make it 1 at that bit position.
...
// but as we have 1s in 1st and 0th bit we can manipulate the number of 1s using nums[i] AND (nums[i] XOR x) and thats
// why just making or them give us 1 in ans.
